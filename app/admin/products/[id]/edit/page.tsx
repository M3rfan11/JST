  ...
  const loadProduct = async () => {
    try {
      ...
      console.log("Loaded product data:", {
        ...
        mediaUrls: product.mediaUrls ? (typeof product.mediaUrls === 'string' ? product.mediaUrls.substring(0, 100) : 'array') : null
      })
      ...
      if (product.isActive !== undefined) {
        ...
        isActive = product.status === 1 || product.status === 'Active' || product.status === '1'
      }
      ...
      if (product.mediaUrls) {
        try {
          const mediaUrls = typeof product.mediaUrls === 'string' 
      ...
      if (product.variantAttributes) {
        try {
          const attrs = typeof product.variantAttributes === 'string' 
      ...
      if (product.variants && Array.isArray(product.variants)) {
        ...
        if (variantAttributes.length === 0 && loadedVariants.length > 0) {
          ...
          if (attrMap.size > 0) {
            setVariantAttributes(Array.from(attrMap.entries()).map(([name, values]) => ({
              name,
        ...
        try {
          const variantsData = await api.productVariants.getByProduct(parseInt(productId)) as any[]
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const updateData = {
        ...
        variantAttributes: variantAttributes.length > 0 ? JSON.stringify(variantAttributes) : null,
      ...
      try {
        ...
        const existingVariants = await api.productVariants.getByProduct(parseInt(productId)) as any[]
        const existingVariantIds = existingVariants.map(v => v.id)
        console.log("📋 Existing variants:", existingVariants.length, existingVariantIds)
        ...
        for (const variant of variants) {
          ...
          if (!color && variant.attributes && typeof variant.attributes === 'object') {
            // Try to get color from attributes
            color = variant.attributes['Color'] || variant.attributes['color'] || Object.values(variant.attributes)[0] || ""
          }
          
          const attributesString = typeof variant.attributes === 'string' 
            ? variant.attributes 
            : JSON.stringify(variant.attributes || {})
          
          const hasAttributes = variant.attributes && typeof variant.attributes === 'object' && Object.keys(variant.attributes).length > 0
          const shouldCreate = color || hasAttributes || attributesString !== '{}'
          ...
          if (variant.id && existingVariantIds.includes(variant.id)) {
            ...
            try {
              ...
              toast({
                title: "Error",
                description: `Failed to create variant: ${createError.message || 'Unknown error'}`,
                variant: "destructive",
              })
            }
          } else {
            console.warn("⏭️ Skipping variant creation - no color or attributes:", variant)
            skippedCount++
          }
        }
        ...
        toast({
          title: "Warning",
          description: "Product updated, but there was an issue managing variants. Please check them manually.",
          variant: "default",
        })
      }
  ...
  if (loadingProduct) {
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-lg" style={{ fontFamily: '"Dream Avenue"' }}>Loading product...</div>
      </div>
    )
  }
            ...
            <div className="space-y-4">
              <div>
                ...
                <p className="text-sm text-muted-foreground mb-4" style={{ fontFamily: '"Dream Avenue"' }}>
                  Upload images from your computer or add image URLs. First image will be used as the primary product image.
                </p>
              </div>
              ...
              {images.length > 0 ? (
                <div>
                  <p className="text-sm text-muted-foreground mb-2" style={{ fontFamily: '"Dream Avenue"' }}>
                    {images.length} {images.length === 1 ? 'image' : 'images'} {images.length === 1 ? 'loaded' : 'loaded'}
                  </p>
            ...
            <div className="space-y-4">
              ...
              {generatedVariants.length > 0 ? (
                <div className="border rounded-md p-4 bg-gray-50">
                  <div className="flex items-center justify-between mb-3">
                    <p className="text-sm font-medium" style={{ fontFamily: '"Dream Avenue"' }}>
                      {generatedVariants.length} variant(s) will be created:
                    </p>
                    <Button
                      type="button"
                      onClick={handleApplyGeneratedVariants}
                      style={{ backgroundColor: '#3D0811', color: 'white', fontFamily: '"Dream Avenue"' }}
                    >
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                    {generatedVariants.slice(0, 12).map((variant, index) => (
                      <div key={index} className="text-xs p-2 bg-white rounded border" style={{ fontFamily: '"Dream Avenue"' }}>
                        {variant.attributes && typeof variant.attributes === 'object' && Object.entries(variant.attributes).map(([key, value]) => (
              ...
              {variants.length > 0 && (
                <div className="border rounded-md p-4 space-y-3">
                  <h4 className="font-medium" style={{ fontFamily: '"Dream Avenue"', color: '#3D0811' }}>
                    Existing Variants ({variants.length})
                  </h4>
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {variants.map((variant, index) => (
                      <div key={variant.id || index} className="border rounded-md p-3 bg-gray-50">
                        {editingVariantIndex === index ? (
                          <div className="space-y-2">
                            <Input
                              ...
                              onChange={(e) => {
                                ...
                                updated[index] = { ...updated[index], priceOverride: e.target.value ? parseFloat(e.target.value) : undefined }
                                setVariants(updated)
                              }}
                              style={{ fontFamily: '"Dream Avenue"' }}
                            />
                          ...
                          <div className="space-y-2">
                                ...
                                {variant.attributes && typeof variant.attributes === 'object' ? (
                                  <div className="text-sm">
                                    {Object.entries(variant.attributes).map(([key, value]) => (
                                      <div key={key} style={{ fontFamily: '"Dream Avenue"' }}>
                            ...
                            {variant.isActive === false && (
                              <span className="text-xs text-red-500" style={{ fontFamily: '"Dream Avenue"' }}>
                                Inactive
                              </span>
                            )}
                          </div>
